...
- 来源列表归属以真实投递为准
- **source 段落的 link 必须指向该段落所在的具体邮件（如 docs 补丁本身），非系列封面**——读者点开链接要能直接看到这段原文（2026-08-11 修正：曾指向封面导致找不到段落）（如该系列投 linux-kernel/virtualization，标 `lkml`，勿标未投递的列表）
...
divider       → "✍️ 今日练习"
                exercise  → text=题目 / answer=参考答案（点击展开）/ source=原文内容贴出（英文原句，非链接）/ link=原文引用（lore 可溯源）
closing       → 每日一句（英文学习收尾）
```
